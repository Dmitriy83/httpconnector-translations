...
**Module-header drift check** (`check_module_header_drift.py`) — detects when literal facts in the module-level docstring (copyright year, version number) drift between the RU source and the frozen English translation in `Module_en.trans` (`Description=` key). EDT treats the module-level docstring as a single prose blob keyed only by `Description=`, so the EN value is hand-written once and never auto-refreshed. Run after every rebuild; if it reports drift, edit the `Description=` value in `.trans` to match the current RU.
