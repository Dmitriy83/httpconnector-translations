...
`scripts/postbuild_patch.py` — Python-скрипт, который правит BSL-файлы переведённого проекта **после** сборки зависимого перевода в EDT. Он существует потому что у EDT LanguageTool есть несколько особенностей, которые **нельзя починить одним только словарём**.
...
1. Скопировать `scripts/postbuild_patch.py`.
